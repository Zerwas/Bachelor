...
The next goal is to connect Azema's inequality with the measure concentration function. To achieve this we first need to introduce a new property of mm-spaces.
\begin{definition}\label{def:length}
Let $\boldsymbol{X}=(X,d,\mu)$ be an mm-space. The \define{length} of $\boldsymbol X$ is the infimum over all $l$ with the following property. There is a refining sequence of partitions 
...
\begin{lemma}
Let $\boldsymbol{X}$ be an mm-space. Then the length of $\boldsymbol{X}$ is at most the diameter of $\boldsymbol{X}$.
\end{lemma}
\begin{proof}
