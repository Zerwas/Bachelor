...
\[\alpha_{\boldsymbol{X}}(\varepsilon)\leq 2\exp\left(-\frac{\varepsilon^2}{16\sum_{i=1}^{n}a_i^2}\right).\]
\end{corollary}
\begin{proof}
...
\[\lim_{n\to\infty} \alpha_{\SL_{2^n}(q)}(r)=0 .\]\FlorianSagt{Boldsymbol or tuple}
\end{theorem}
\begin{proof}
...
where $\SL_{i-1}(q)$ becomes a subgroup of $\SL_i(q)$ via the embedding $g\mapsto g\oplus 1_{\langle e_i\rangle}$. Next we want to bound the diameter of $\SL_{i}(q)/\SL_{i-1}(q)$ by $\frac{3}{n}$. By Lemma \ref{lem:factorMetric} it suffices to show that for any $g,g'\in\SL_i(q)$ there is a $h\in \SL_{i-1}(q)$ such that $d(g,g'h)\leq\frac{3}{n}$. Since $d$ is bi-invariant we can assume w.l.o.g. that $g'=1_V$.

$U'=\langle e_1,\dots,e_{i-1}\rangle$, $U=\langle e_i\rangle$, $U''=\langle e_{i+1},\dots, e_n\rangle$

\begin{align*}
d(g,(h'\oplus 1)g)=d(1_V,h'\oplus 1)\leq \frac{2}{n}
\end{align*}

$h:=(1_{U'}\oplus 0_U)(h'\oplus 1)g+ (0_{U'}\oplus1_{U})$, $h\in\SL_{i-1}$?
\begin{align*}
d((h'\oplus 1)g,h)
\end{align*}
\end{proof}
