...
\end{lemma}
\begin{proof}
The first claim is clear from the definition. For the second one note that $U\subseteq U^{\bot\bot}$. By Lemma \ref{lem:dimUBot} they also have the same dimension and are therefore equal.
\end{proof}
...
Then $r\mapsto r', s\mapsto \lambda s'$ with $\lambda=\omega'(r',s')^{-1}$ is an isomorphism.
\end{proof}
Next we will strengthen this result and show that a symplectic space is uniquely determined by its dimension.
...
For the remainder of this section let $\alpha\colon U\to W$ be an isometrie between subspaces $U,W\leq V$.
\begin{lemma}
