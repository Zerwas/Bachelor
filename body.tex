...
Next we will apply the methods from the previous section to show that $\clim \SL_{2^n}(q)$ is extremely amenable.
...
\end{lemma}
\begin{proof}
	If $U$ is degenerate, then there is a $u\in U$ such that $u\bot v$ for all $v\in U$. Therefeore $u\in U^\bot$. Vice versa if $u\in (U\cap U^\bot)\setminus\{0\}$, then $\omega(u,v)=0$ for all $v\in U$.
\end{proof}
...
\begin{corollary}\label{cor:structureSympl}
	...
	Let $(V,\omega)$ be a symplectic vector space. Then $V$ is of even dimension $2n$ for some $n\in\N$ and there are hyperbolic planes $U_1,\dots, U_n\leq V$ such that $V=U_1\oplus\dots\oplus U_n$ and $U_i\bot U_j$ for $i\not=j$.
