...
\begin{theorem}[Theorem \textbf{3.1.3} in \cite{AzemaLevy}]\label{thm:LevyImpliesExAm}
...
As the next results rely heavily on stochastic methods we will briefly introduce the necessary notions. Since the $G_n$ are all finite and equipped with the normalized counting measure we will only consider \define{probability spaces} $(X,\Sigma,\mu)$, where $X$ is finite, $\Sigma$ is a $\sigma$-algebra over $X$, and $\mu(A)=|A|/|X|$ for $A\subseteq X$. In this section we will roughly follow Section 3.2 from \cite{AzemaLevy}. Most of the statements presented in this section hold in a more general setting. 
...
Before we will prove Azuma's inequality, let us introduce some useful notation. Whenever there is no danger of confusion we will abbreviate sets of the form
\begin{align*}
...
\begin{lemma}[Azuma's inequality]\label{lem:azema}
...
\begin{definition}
	Let $(X,d)$ be a metric space and let $\sim$ be an equivalence relation on $X$. Then \[d_\sim([x],[y])=\inf\{d(p_1,q_1)+\dots+d(p_n,q_n)\mid q_i\sim p_{i+1}, x\sim p_1, q_n\sim y, n\in\N\}\] defines a pseudo-metric on $X/{\sim}$. We call  $d_\sim$ the \define{factor metric} on $X/\sim$.
\end{definition}
...
%In the case of $\SL_n(q)$ an $n$ dimensional $\F_q$ vector space $V$ suffices. Fixing a basis $e_1,\dots,e_n$ gives us an embedding from $\SL_n(q)$ into $\Aut(V)$. 
...
\begin{definition}
	Let $V$ be an $\F_q$-vector space. A bilinear form $\omega$ on $V$ is called \define{symplectic} if $\omega$ is nondegenerate, i.e.\ for all $x\in V\setminus\{0\}$ there is an $y\in V$ such that $\omega(x,y)\not=0$, $\omega(x,y)=-\omega(y,x)$ for all $x,y\in V$, and if $q=2$, then $\omega(x,x)=0$ for all $x\in V$. For $x,y\in V$ with $\omega(x,y)=0$ we often write $x\bot y$. We call $(V,\omega)$ a \define{symplectic vector space}.
				
	A finite group $G$ is called \define{symplectic} if there is a symplectic vector space $(V,\omega)$ such that $G\cong\Aut(V,\omega)$, if $V$ is $n$-dimensional, then we will denote $\Aut(V,\omega)$ by $\Sp_n(q)$. 
...
\begin{lemma}\label{lem:nondegenCharacterization}
...
In the following we want to investigate the structure of symplectic vector spaces.
We start with 2-dimensional symplectic vector spaces, which we will call \define{hyperbolic planes}.
...
%\end{corollary}
With these powerful tools we can prove Witt's Lemma easily.
...
\begin{definition}
	...
	Then $(V,\omega)$ is an \define{orthogonal vector space} if $\omega$ is bilinear, $\omega(x,y)=\omega(y,x)$ for all $x,y\in V$, and if $q=2$ then $\omega(x,x)=0$ for all $x\in V$.
				
	And $(V,\omega)$ is a \define{unitary vector space} if there is an $h\in\Aut(\F_q)$ with $h^2=1$ such that 
	\begin{align*}
	...
	\define{Orthogonal} and \define{unitary groups} are the automorphism groups of unitary and orthogonal vector spaces, respectively.
\end{definition}
